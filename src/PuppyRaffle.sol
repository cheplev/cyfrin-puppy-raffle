...
contract PuppyRaffle is ERC721, Ownable {
    ...
    // @audit add check if _feeAddress is not address(0)  ?
    ...
    function enterRaffle(address[] memory newPlayers) public payable {
        ...
        // @audit: at first we check if the user is already exist and then we push him to the array
    ...
    // @audit: Should we call here getActivePlayerIndex? 

    function refund(uint256 playerIndex) public {
        address playerAddress = players[playerIndex];
        require(playerAddress == msg.sender, "PuppyRaffle: Only the player can refund");

        // @audit Do we need to check it? we already check if the address is real;
    ...
    // @audit: add function for getting the index of the user without parameters
    ...
    /// @dev we use a hash of on-chain data to generate the random numbers // @audit  ?????
    /// @dev we reset the active players array after the winner is selected
    /// @dev we send 80% of the funds to the winner, the other 20% goes to the feeAddress

    // @audit: if anyone can call this function? Can I just calculate the winnerIndex myself and call the function if I like the index?
    // or can I find the neccessary index by myself?
    ...
    // @audit: add check if newFeeAddress is not address(0) ?
